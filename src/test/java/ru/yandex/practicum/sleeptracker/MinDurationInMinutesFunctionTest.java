...
import ru.yandex.practicum.sleeptracker.function.MinDurationInMinutesFunction;
...
public class MinDurationInMinutesFunctionTest {

    private List<SleepingSession> listSleepingSession;
    private static MinDurationInMinutesFunction function;
    private static SleepingSession session0;
    private static SleepingSession session1;

    @BeforeAll
    public static void setup() {
        function = new MinDurationInMinutesFunction();
    ...
    @Test
    void testMinDurationInMinutesWithNoSession() {
        SleepAnalysisResult result = function.apply(listSleepingSession);
        assertEquals(0, result.getResult());
        assertEquals("Минимальная длительность сессии (в минутах)", result.getDescription());
    }

    @Test
    void testMinDurationInMinutesWithOneSession() {
    ...
    @Test
    void testMinDurationInMinutesWithTwoSession() {
        listSleepingSession.add(session0);
